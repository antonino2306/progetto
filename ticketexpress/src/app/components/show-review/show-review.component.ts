...
export class ShowReviewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ticketsSubscription = this.userService.tickets.subscribe((tickets) => {
      ...
      this.showTicketExpired = this.ticketExpired.filter(
        (ticket, index, self) =>
  ...
  filteringTicketsExp(event: any) {
    ...
    if (ricerca === '') {
      ...
      this.ticketExpired = this.ticketExpiredOriginal.filter((ticket) => {
        ...
        const nomeCompleto = `${ticket.holderName || ''} ${
          ticket.holderSurname || ''
        }`.toLowerCase();

        // Ricerca per Cognome Nome
        const cognomeNome = `${ticket.holderSurname || ''} ${
          ticket.holderName || ''
        }`.toLowerCase();
      ...
      const showIDsWithMatchingTickets = [
        ...new Set(this.ticketExpired.map((ticket) => ticket.showID)),
      ];
